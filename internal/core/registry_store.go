...
func validateBuiltinInfo(skill SkillManifest) error {
	...
	for _, backend := range skill.Builtin.Backends {
		...
		case "onnxruntime", "ncnn", "net_http", "search_http", "openxml", "pdf_text":
		default:
	...
	for _, profile := range skill.Builtin.ModelProfiles {
		...
		case "ppocrv6", "ppocrv5", "ppocrv4", "readability_v1", "search_results_v1", "docx_v1", "xlsx_v1", "pptx_v1", "pdf_text_v1":
		default:
