...
func DefaultCapabilityPacks() []CapabilityPack {
	return []CapabilityPack{
		...
		},
		{
			...
			Tier:            "first_wave",
			CapabilityClass: "tool",
			UseWhen:         "Extract text from screenshots, scans, rendered PDF page images, UI images, or photos with RapidOCR latest ONNX profiles.",
			Summary:         "RapidOCR-compatible native OCR for screenshots, scans, rendered page images, and UI text, built for document work and automation.",
			Description:     "Use when text must be recovered from image-like inputs with structure, coordinates, confidence notes, or RapidOCR model selection.",
			Inputs:          []string{"image file or rendered page image", "optional language hints", "optional model profile such as rapidocr", "optional engine preference"},
